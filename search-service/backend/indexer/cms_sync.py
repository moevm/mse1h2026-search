...
class CMSExtractor:
    ...
    def _fetch_tv_values(self, cursor) -> dict:
        ...
        for row in tv_rows:
            ...
            if not field:
                continue
            val = _parse_persons(row['value']) if field == 'tv_persons' else row['value']
            if val:
    ...
    def _process(
        ...
        for row in rows:
            ...
            if row.get('publishedon'):
                row['published_year'] = str(datetime.fromtimestamp(row['publishedon']).year)
            elif row.get('createdon'):
                row['published_year'] = str(datetime.fromtimestamp(row['createdon']).year)
            else:
                row['published_year'] = ''
