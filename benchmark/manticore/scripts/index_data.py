...
from config import (
    MYSQL_CONFIG, MANTICORE_URL, MANTICORE_TABLE,
    CHUNK_SIZE, logger, EMBEDDER_MODEL_NAME, MODEL_CACHE_DIR
)
...
def build_url(doc_id: int, docs_dict: Dict[int, Any], visited_ids: Set[int] = None) -> str:
...
def setup_manticore_table(recreate: bool) -> None:
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict):
            ...
            if error_msg:
                logger.error(
                    f"Критическая ошибка создания таблицы: {error_msg}")
                sys.exit(1)
    except Exception:
        pass
...
def _extract_semantic_text(pagetitle: str, description: str, intro: str, content: str) -> str:
...
def process_documents(all_docs: List[Dict[str, Any]], embedder) -> List[Dict[str, Any]]:
    ...
    for doc in tqdm(all_docs, desc="1/3 Очистка HTML и сбор данных", unit="doc"):
        if not (doc['searchable'] == 1 and doc['privateweb'] == 0 and doc['type'] in ['document', 'reference']):
            continue
    ...
    documents_to_insert = [
        ...
        for doc, vector in zip(valid_docs, tqdm(vectors_generator, total=len(valid_docs), desc="2/3 Нейросетевая векторизация", unit="doc"))
    ]
